...
def receive():
    ...
    if nav_button == "out": #go out of form
    ...
    if form.validate_on_submit():
        ...
        if session['delete_id']:
            dltsku = sel.all(db, 'logix_de', 'id', 
                             id = session['delete_id'])[0].get('log_sku')
            idstckdupd = sel.secondfromtop(db, 'logix_de', 
...
def returns():
    ...
    if rtnrcdexists:
        ...
        #and internal fields
        for i in form:
            if not (type(i).__name__ in ('CSRFTokenField', 'HiddenField')):
                i.render_kw={'disabled':True}
                if type(i).__name__ == 'FormField':
                    for ii in i:
